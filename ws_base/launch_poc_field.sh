#!/bin/bash
# launch_poc_field.sh — Multi-Domain POC Experiment Launcher  [ON-FIELD MODE]
#
...
cleanup() {
    ...
    [[ -n "$TOPIC_BW_D4_PID"    ]] && kill -TERM "$TOPIC_BW_D4_PID"    2>/dev/null || true
...
start_latency_collectors() {
    ...
    TARGET_HOST="$JETSON_HOST" TARGET_LABEL=jetson_d6 SSH_OPTS="$SSH_OPTS" \
        bash "$TOOLS_DIR/tracing/start_trace_d6.sh" || warn "  Jetson D6 latency collector failed to start (non-fatal)"
...
start_topic_bw_collector() {
    ...
    TOPIC_BW_D4_PID=$!
    BACKGROUND_PIDS+=("$TOPIC_BW_D4_PID")
    sleep 1
    if ! kill -0 "$TOPIC_BW_D4_PID" 2>/dev/null; then
        warn "Topic BW (D4) collector exited immediately — check $LOG_DIR/topic_bw_d4.log"
        TOPIC_BW_D4_PID=""
    else
...
print_summary() {
    ...
    echo ""
    echo "  Run directory : $RUN_DIR"
    echo "  Mode: ON-FIELD | Branch: multi-domain | Domains: D5/D4/D6 | Duration: ${RUN_DURATION}s"
    echo ""
    echo "  Raw CSVs:"
...
main() {
    ...
    echo ""
    echo -e "${BOLD}  Multi-Domain POC — ON-FIELD MODE${NC}"
    echo -e "${RED}  Camera: LIVE D415 ONLY (no video fallback)${NC}"
    echo -e "  Params: $PARAMS_DIR"
    echo -e "  Branch: multi-domain | Domains: D5/D4/D6 | Duration: ${RUN_DURATION}s"
