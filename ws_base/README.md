...
| `launch_base_single_domain.sh` | Starts base-station nodes only (all D5, tmux) | Base nodes only — RPi and Jetson already running |
| `launch_base_tmux.sh` | Base nodes in dual-domain (D5 command + D4 monitoring, tmux) | Non-POC base session |
...
Results saved to `ws_base/runs/single_domain/run_NNN/`.
...
See [POC_SINGLE_DOMAIN.md](../POC_SINGLE_DOMAIN.md) for the full measurement guide
...
```bash
cd ~/almondmatcha/ws_base
bash launch_base_single_domain.sh   # D5 only (POC topology)
```
