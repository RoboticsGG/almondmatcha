#!/bin/bash
# launch_poc_lab.sh — Single-Domain POC Experiment Launcher  [LABORATORY MODE]
#
...
#   5. Start latency collectors on RPi and Jetson (D5)
#   6. Start net/CPU/SoftIRQ collectors + D5 topic BW collector
...
#   ws_base/runs/single_domain/run_NNN/merged_all.csv
...
launch_ros2_nodes() {
    ...
    log "  All ROS2 nodes launched — D5 domain"
    log "  Total: 12 Linux nodes (D5: 12) staggered over ~35 s"
}
...
print_summary() {
    ...
    echo ""
    echo "  Run directory: $RUN_DIR"
    echo "  Mode: LABORATORY | Branch: single-domain | Domain: D5 | Duration: ${RUN_DURATION}s"
    echo ""
    echo "  Raw CSVs:"
    for f in latency_rpi.csv latency_jetson.csv \
...
main() {
    ...
    echo ""
    echo -e "${BOLD}  Single-Domain POC — LABORATORY MODE${NC}"
