#!/bin/bash
# Rover Launch with Tmux - 3x2 Grid Layout (5 nodes, all in Domain 5)
# Domain 5: All rover nodes for sensor fusion (GNSS, chassis, sensors, future EKF)
...
# Pane 1 (middle-left): GNSS Mission Monitor (Domain 5)
...
# Pane 2 (bottom-left): Chassis Controller (Domain 5)
...
# Pane 5 (bottom-right): Reserved for future EKF or monitoring
tmux select-pane -t 5 -T "Reserved_EKF"
tmux send-keys -t $SESSION_NAME:0.5 "cd ~/almondmatcha/ws_rpi && source install/setup.bash" C-m
tmux send-keys -t $SESSION_NAME:0.5 "clear && echo -e '\\e[1;37m>>> [RESERVED] Future: EKF Sensor Fusion (Domain 5) <<<\\e[0m'" C-m
