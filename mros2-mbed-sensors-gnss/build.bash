...
# Docker runs as root → all generated files (mbed-os/, mros2/, build/, platform/) are
# owned by root.  Fix ownership back to the invoking user so the next run works without sudo.
# platform/ contains templates.hpp which is regenerated by cmake's templates_generator.py.
if [ -n "${SUDO_USER:-}" ]; then
