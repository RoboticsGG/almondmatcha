...
**Domain 4**: Telemetry — `mission_monitoring_node_pc` (Base, display) + `node_rover_local_monitoring` (Jetson, CSV)  
...
graph TB
    ...
    subgraph D5["Domain 5 — Control Network"]
        ...
        SPRES["node_gnss_spresense<br/>RPi"]
        RTK["node_gnss_ublox<br/>RPi"]
        NAV["node_gnss_mission_monitor<br/>RPi"]
        MON["mission_monitoring_node_rpi<br/>RPi — D5 sub / D4 pub"]
...
- Jetson (`node_rover_local_monitoring`): unified CSV from D4 relay at 5 Hz, stored in `ws_jetson/runs/`
...
# RPi: node_chassis_controller, node_chassis_imu, node_chassis_sensors,
...
| `tpc_rover_dest_coordinate` | Event | RPi (node_gnss_mission_monitor) | Mission destination waypoint |
...
**Data logging:** `mission_monitoring_node_rpi` (RPi) logs 6 per-topic CSV files at native rates (4–50 Hz) in `ws_rpi/runs/`. `node_rover_local_monitoring` (Jetson, D4) logs aggregated telemetry at 5 Hz in `ws_jetson/runs/`. See [docs/CSV_LOGGING.md](docs/CSV_LOGGING.md).
...
**Naming Conventions:**
- **C++ Nodes:** Use `node_*` prefix (e.g., `node_chassis_controller.cpp` → executable `node_chassis_controller`)
...
**Contributing:** Follow ROS2 naming conventions (`node_*` for C++ nodes, `*_node.py` for Python, `tpc_*` for topics), write descriptive commits, update documentation for architectural changes, test on all platforms before merging.
