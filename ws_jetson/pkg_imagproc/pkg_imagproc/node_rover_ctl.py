...
class ExponentialMovingAverageLPF:
    def __init__(self, alpha, maxlen=30):
        self.alpha = alpha
        self.ema = None
        self.maxlen = maxlen                   
...
class RoverCtlNode(Node):
    def __init__(self):
        ...
        # ---- Control gains ----
        self.declare_parameter('k_e1', 1.0)     # weight for zeta (heading error)
        self.declare_parameter('k_e2', 0.1)     # weight for b (lateral offset)
        ...
        self.csv_path = os.path.join(log_dir, "rover_ctl_log_ver_3.csv")
    ...
    def lane_callback(self, msg: Float32MultiArray):
        ...
        # PID control with wall-clock dt
        now = time.time()
        ...
        # Derivative term
        d_term = (e_sum - self.last_error) / dt if dt > 1e-6 else 0.0
        self.last_error = e_sum

        u = (self.k_p * e_sum) + (self.k_i * self.integral) + (self.k_d * d_term)
        # u = 55
        # Steering (deg)
        steer_angle = u
