...
def perspective_transform(binary, frame_size):
    ...
    ROI_BASE = np.float32([[0, 500],[1232, 500],[768, 180],[410, 180]])
...
def compute_lane_params(binary_warped):
    ...
    if len(leftx) >= 50 and len(rightx) >= 50:
        m_left,  b_left  = np.polyfit(lefty,  leftx,  1)
        m_right, b_right = np.polyfit(righty, rightx, 1)
        ...
        b_centered = b_center - (W // 2)

        results = {"theta": theta, "b": b_centered, "detected": True}
    
    elif len(leftx) >= 50:
        m_left, b_left = np.polyfit(lefty, leftx, 1)
        theta = np.degrees(np.arctan(m_left))
        b_centered = b_left - (W // 2)
        results = {"theta": theta, "b": b_centered, "detected": True}
    
    elif len(rightx) >= 50:
        m_right, b_right = np.polyfit(righty, rightx, 1)
        theta = np.degrees(np.arctan(m_right))
        b_centered = b_right - (W // 2)
        results = {"theta": theta, "b": b_centered, "detected": True}
