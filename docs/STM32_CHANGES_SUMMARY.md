...
**Branch:** `single-domain` (POC — all nodes on D5) | **Base:** mROS-base/mros2 v0.5.4  
...
Single-domain POC: **16 D5 participants** (vs 11 in multi-domain) — proxy pools cover all 16.

| Parameter | Default | SDM value | Reason |
|-----------|---------|-----------|--------|
| `MAX_NUM_PARTICIPANTS` | 15 | **1** | Pool of 15 = 525 KB BSS → OOM at boot |
| `SPDP_MAX_NUMBER_FOUND_PARTICIPANTS` | 14 | **30** | 16 nodes + daemons + CLI tools + margin |
| `NUM_WRITER_PROXIES_PER_READER` | 6 | **30** | 1 per remote participant, matches SPDP_MAX |
| `NUM_READER_PROXIES_PER_WRITER` | 6 | **30** | Same |
| `THREAD_POOL_READER_STACKSIZE` | 4096 | **8192** | 4096 → HardFault at 99.6% stack depth |
| `THREAD_POOL_WORKLOAD_QUEUE_LENGTH` | 20 | **40** | Absorb 16-participant discovery burst |
...
| `DOMAIN_ID` | 0 | **5** | Single-domain POC (all nodes on D5) |
...
|-----------|---------|--------|--------|
| `lwip.pbuf-pool-size` | 16 | **32** | 16-participant burst needs more pbufs than 11-participant multi-domain |
| `lwip.socket-max` | 4 | **8** | RTPS multicast + unicast sockets |
